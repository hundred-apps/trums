    ...
    <el-card class="my-3">
      <template #header>
        <div class="card-header">
          <el-form-item>
            <el-button type="primary" :loading="loading" @click="submitForm(ruleFormRef)">Simpan</el-button>
        ...
        <el-table-column label="Referensi" >
          ...
          <el-table-column label="Nomor/Nama Item" >
              <template #default="{ row, $index }">
                  <el-autocomplete
                      ...
                      :fetch-suggestions="(query, cb) => querySearchReference(query, cb, row.reference)"
                      placeholder="Cari nomor referensi"
                      class="w-full"
                      :disabled="row.reference == null"
                      @select="(selected) => onHandleSelectReference(selected, $index, row.reference)"
                  >
                      <template #default="{ item: suggestion }">
                      <div v-if="suggestion.isNew" class="flex items-center text-blue-500">
                          <el-icon><Plus /></el-icon>
                          <span class="ml-2">Tambahkan "{{ suggestion.value }}"</span>
                      </div>
                      <div v-else>
                         <p style="line-height: 15px;" class="font-bold">{{ suggestion.customer_name }}</p>
                          <p>Nomor: {{ suggestion.unique_code ?? 'Tidak Ada' }} | Jumlah: {{currency(suggestion.total_amount) ?? 'Tidak Ada'}}</p>
                          
                      </div>
                      </template>
                  </el-autocomplete>
                  
                  <el-input v-else v-model="row.reference_value" :disabled="row.reference == null" placeholder="Nama item" />
              </template>
          </el-table-column>
          ...
          <template #default="{ row, $index }">
              <el-input-number
                  ...
                  controls-position="right"
                  class="w-full"
                  :disabled="row.reference == null || row.reference == 'invoice' || row.reference == 'bill'"
                  @change="calculateItemAmount($index)"
              />
          </template>
          </el-table-column>
...
const rules = reactive<FormRules>({
  type: [{ required: true, message: 'Jenis transaksi wajib dipilih', trigger: 'change' }],
  date: [{ required: true, message: 'Tanggal wajib diisi', trigger: 'change' }],
  account_id: [{ required: true, message: 'Akun wajib dipilih', trigger: 'change' }],
...
const deleteItem = async (unique_id: string, index: number) => {
  loading.value = true;
  try {
    const response = await useFetchApi<BaseResponse<any>>('/transaction-items-delete', 'delete_item', 'post', [unique_id]);
...
const querySearchReference = (query: string, cb: (arg: any) => void, referenceType: string | null) => {
  try {
    if (!referenceType) {
      cb([])
      return
    ...
    if (referenceType === 'invoice') {
      ...
      useFetchApi<ResponsePagination<Invoice[]>>('/search', 'inovices', 'post', request_search).then((response) => {
        if(response.status.value == 'success'){
          const invoices: Invoice[] = (response.data.value?.data ?? []) as Invoice[];
            cb(invoices.map(inv => ({
      ...
      useFetchApi<ResponsePagination<Catalogue[]>>('/search', 'search-catalogue', 'post', request_search).then((response) => {
        if(response.status.value == 'success'){
...
async function createCatalogue(data: any): Promise<Catalogue|null>  {
  try {
    const response = await useFetchApi<BaseResponse<Catalogue|null>>('/catalogues-create', 'create-catalogue', 'post', data);
    if(response.status.value == 'success'){
...
const onHandleSelectReference = async (selected: any, index: number, reference: string) => {
  const item = ruleForm.transaction_items[index]
  ...
  if ((selected.reference === 'invoice' || selected.reference === 'bill')) {
      ...
      if (selected.isNew) {
        const catalogue: Catalogue|null = await createCatalogue({name: selected.data});
    ...
    if (valid) {
      ...
      try {
        ...
        ruleForm.transaction_items.forEach((value, index) => {
          formData.append(`transaction_items[${index}][unique_id]`, value.unique_id ?? '')
          formData.append(`transaction_items[${index}][reference]`, value.reference ?? '')
          formData.append(`transaction_items[${index}][reference_id]`, (value.reference_id ?? ''))
          formData.append(`transaction_items[${index}][reference_value]`, value.reference_value)
          formData.append(`transaction_items[${index}][description]`, value.description ?? '')
          formData.append(`transaction_items[${index}][quantity]`, `${value.quantity}`)
          formData.append(`transaction_items[${index}][price_per_unit]`, `${value.price_per_unit}`)
          formData.append(`transaction_items[${index}][amount]`, `${value.amount}`)
        })
        ...
        const response = await useFetchApi<BaseResponse<Transaction>>('/transaction-create', 'create-transaction', 'post', formData);
        
        if(response.status.value === 'success'){
          const transaction: Transaction|null = response.data.value?.data as Transaction;
          if(transaction){
        ...
        console.error('Error saat membuat transaksi:', error.response.message ?? error);
...
const fetchDataEdit = async () => {
  ...
  try {
    const unique_id = useCookie('unique_id');
    const response = await useFetchApi<BaseResponse<Transaction>>(`/transaction-read/${unique_id.value}`, 'data-edit', 'get', null);
    if(response.status.value == 'success'){
