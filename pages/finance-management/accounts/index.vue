...
// Data state
const {data} = await useFetchApi<ResponsePagination<Account[]>>(`/search`, 'account', 'post', request_search.value);
...
const column_selected = ref<string[]>(['selection', 'code', 'name', 'type', 'created_at', 'operations', 'setup'])
...
const filteredColumn = computed(() => {
  return columnAccount.filter(col => column_selected.value.includes(col.key!.toString()))
})

// Definisi kolom tabel
const columnAccount: Column<Account>[] = [
  {
    key: 'code',
    title: 'Kode Akun',
    dataKey: 'code',
    width: 150,
    cellRenderer: ({rowData: row}) => (<NuxtLink href={`/finance-management/accounts/${row.unique_id}`} class={"text-blue-500"}>{row.code}</NuxtLink>)
  },
  {
    key: 'name',
    title: 'Nama Akun',
            ...
            default: () => (
              <div class="filter-wrapper">
                <div class="filter-group flex flex-col">
                  <ElCheckbox value="asset" v-model={request_search.value.column[0].type}>
                    Asset
                  </ElCheckbox>
                  <ElCheckbox value="liability" v-model={request_search.value.column[0].type}>
                    Liability
                  </ElCheckbox>
                  <ElCheckbox value="revenue" v-model={request_search.value.column[0].type}>
                    Revenue
                  </ElCheckbox>
...
<template>
  <TrumsWrapper>
    <el-row :gutter="20" class="mb-3">
      ...
      <NuxtLink class="el-button el-button--primary el-button--default" href="accounts/add">
        Tambah Akun Baru
      </NuxtLink>
      <el-button
        size="default"
