<template>
  <TrumsWrapper>
    <el-row :gutter="16">
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).reduce((sum, item) => item.status == PaymentStatus.DRAFT ? sum + (item.total_amount || 0) : sum, 0)" title="Invoice Draft">
            <template #title>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).reduce((sum, item) => item.status == PaymentStatus.RECEIVED ? sum + (item.total_amount || 0) : sum, 0)" title="Invoice Received">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                Invoice Received
              </div>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).reduce((sum, item) => item.status == PaymentStatus.UNPAID ? sum + (item.total_amount || 0) : sum, 0)" title="Invoice Paid">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                Invoice Unpaid
              </div>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).reduce((sum, item) => item.status == PaymentStatus.PAID ? sum + (item.total_amount || 0) : sum, 0)" title="Invoice Paid">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                Invoice Paid
              </div>
    ...
    <div class="grid grid-cols-6 gap-4 my-3">
      <el-button size="default" class="button-new-tag" :icon="Plus" @click="visibleFilter = true">Filter</el-button>
      <template v-for="(tag, i) in request_search.column" :key="i">
        <el-tag size="large" @close="handleClose(key, 'column')" v-for="[key, value] in Object.entries(tag).filter(([k, v]) => k !== 'type' && !filterIsEmpty(v) )" closable type="info">
          {{ columns.find((value) => value.dataKey == key)?.title }}
        </el-tag>
      </template>
      <el-tag size="large" @close="handleClose(key, 'filter')" v-if="request_search.filter != null" v-for="[key, value] in Object.entries(request_search.filter).filter(([k, v]) => k !== 'type' && !filterIsEmpty(v) )" closable type="info">
        {{ columns.find((value) => value.dataKey == key)?.title }}
      </el-tag>
      ...
      <el-drawer v-model="visibleFilter" :show-close="false">
        ...
        >
          ...
          <el-form-item label="Invoice Total">
            <el-col :span="11">
              <el-input-number style="width: 100%" v-model="ruleFormFilter.amount_start" :min="1" />
            </el-col>
            <el-col :span="2" class="text-center">
              <span class="text-gray-500">-</span>
            </el-col>
            <el-col :span="11">
              <el-input-number style="width: 100%" v-model="ruleFormFilter.amount_end" :min="1" />
            </el-col>
          </el-form-item>
          <el-form-item label="Status" prop="status">
            <el-select v-model="ruleFormFilter.status" multiple
      collapse-tags placeholder="Select" style="width: 100%" :loading="loadingFilter">
              <el-option
          ...
          <el-form-item label="Metode Pembayaran" prop="payment_method">
            <el-select v-model="ruleFormFilter.payment_method" multiple
      collapse-tags placeholder="Select" style="width: 100%" :loading="loadingFilter">
              <el-option
                v-for="item in paymentStatuses"
                :key="item.value"
...
import { Eleme, SetUp, Filter, Plus, CircleCloseFilled } from '@element-plus/icons-vue'
import { type Column, type CheckboxValueType, TableV2FixedDir, ElPopover, ElCheckbox, ElIcon, type SortBy, subMenuProps, ElCheckboxGroup, type FormInstance, formProps, type FormProps } from 'element-plus'
...
const totalDraft = computed(() => {
  ...
  (data.value?.data ?? []).forEach((value) => {
    if(value.status == PaymentStatus.DRAFT){
...
const columns: Column<Invoice>[] = [
  {
    ...
    cellRenderer: ({ rowData: row }) => (
      <NuxtLink href={`/finance-management/invoice/${row.unique_id}`} class="text-blue-500">
        {row.unique_code}
      </NuxtLink>
    )
  },
  {
    ...
    cellRenderer: ({ rowData }: { rowData: Invoice }) => (
      <span>{rowData.invoice_date ? formatLocalDate(rowData.invoice_date) : '-'}</span>
    )
  },
  {
    key: 'due_date',
    ...
    sortable: true,
    cellRenderer: ({ rowData }: { rowData: Invoice }) => (
      <span>{rowData.invoice_date ? formatLocalDate(rowData.due_date!) : '-'}</span>
    )
  },
  {
    key: 'customer_id',
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }: { rowData: Invoice }) => {
      ...
      return <span>{formatCurrency(rowData.total_amount)}</span>
    }
  },
  {
    key: 'status',
              ...
              <div class="filter-wrapper">
                <div class="filter-group flex flex-col">
                  <ElCheckboxGroup v-model={request_search.value.column[0].status}>
                    <ElCheckbox key={PaymentStatus.DRAFT} value={PaymentStatus.DRAFT} label={PaymentStatus.DRAFTVIEW} />
                    <ElCheckbox key={PaymentStatus.RECEIVED} value={PaymentStatus.RECEIVED} label={PaymentStatus.RECEIVEDVIEW} />
                    <ElCheckbox key={PaymentStatus.PAIDVIEW} value={PaymentStatus.PAIDVIEW} label={PaymentStatus.PAIDVIEW} />
                    <ElCheckbox key={PaymentStatus.UNPAID} value={PaymentStatus.UNPAID} label={PaymentStatus.UNPAIDVIEW} />
                  </ElCheckboxGroup>
                </div>
              </div>
            ),
        ...
        <ElPopover ref={popoverRef} trigger="click" {...{ width: 200 }}>
          {{
            default: () => (
              <div class="filter-wrapper">
                <div class="filter-group flex flex-col">
                  <ElCheckboxGroup v-model={request_search.value.column[0].payment_method}>
                    <ElCheckbox key={PaymentMethod.BankTransfer} value={PaymentMethod.BankTransfer} label={PaymentMethod.BankTransfer.toUpperCase()} />
                    <ElCheckbox key={PaymentMethod.Giro} value={PaymentMethod.Giro} label={PaymentMethod.Giro.toUpperCase()}/>
                    <ElCheckbox key={PaymentMethod.Cash} value={PaymentMethod.Cash} label={PaymentMethod.Cash.toUpperCase()} />
                  </ElCheckboxGroup>
                </div>
              </div>
            ),
...
const onDelete = async (uniques: string[]) => {
  try {
    ...
    const response = await useFetchApi<BaseResponse<any>>('/invoice-delete', 'delete-invoice', 'post', uniques);
...
//   ruleFormFilter.customer_id = customer.unique_id;
//   ruleFormFilter.customer_name = customer.name;
// }
