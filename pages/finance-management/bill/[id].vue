      ...
      <template #header>
        <div class="card-header flex justify-end">
          ...
          <el-button
            type="success"
            v-if="
              invoiceData?.status === PaymentStatus.UNPAID ||
              invoiceData?.status === PaymentStatus.RECEIVED
            "
            @click="markAsPaid"
          >
        ...
        <div class="flex-1">
          <el-descriptions title="" :column="1" size="large" border>
            ...
            <el-descriptions-item label="Pembayaran">
              <p v-if="invoiceData?.payment_term == PaymentTerm.TEMPO">
                {{ invoiceData?.payment_term_value ?? "-" }} ({{
                  invoiceData?.payment_term_unit
                }})
              </p>
              <p v-else>
                {{ (invoiceData?.payment_term ?? "-").toUpperCase() }}
              </p>
            </el-descriptions-item>
...
import { formatLocalDate, displayAmount } from "#imports";
