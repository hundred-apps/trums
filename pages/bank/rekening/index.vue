...
const filteredData = computed(() => {
  ...
  return data.value.data.query.filter(
    (account) =>
      !search.value ||
...
// Columns definition
const columns: ColumnTable<BankAccount>[] = [
  {
    key: "bank_name",
    title: "Nama Bank",
    width: 200,
...
columns.unshift({
  ...
  headerCellRenderer: () => {
    ...
    const allSelected = (data.value?.data.query ?? []).every(
      (row) => row.checked
    );
    const someSelected = (data.value?.data.query ?? []).some(
      (row) => row.checked
    );
...
<template>
  <TrumsWrapper>
    <el-row :gutter="20" class="mb-3">
      ...
      <el-button :loading="loading" :icon="Eleme" @click="handleRefresh">
        Muat Ulang
      </el-button>

      <el-button type="danger" :disabled="!checkSelect()" @click="batchDelete">
