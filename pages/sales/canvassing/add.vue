    ...
    <el-card class="my-3">
      <template #header>
        <div class="card-header">
          <el-form-item>
            <el-button :disabled="loading" type="primary" @click="submitForm(ruleFormRef)">Simpan</el-button>
      ...
      >
        <el-form-item label="Nomor Referensi" prop="source_document">
          <div class="flex gap-2">
            ...
            <el-button :disabled="loading" :icon="Plus" type="primary" size="default" @click="visibleModalRequest = true"/>
          </div>
        </el-form-item>
    ...
    item_canvassing.value.forEach(element => {
      element.children.forEach(child => {
        child.contacts_fee.forEach(contact => {
          const index = contactsFee.value.findIndex((value) => value.party_id == contact.party_id);
          if(index >= 0){
  ...
  try {
    ...
    item_canvassing.value.forEach((item: CanvassingItemForm, i: number) => {
      ...
      item.children.forEach((vendor: CanvassingItemForm, j: any) => {
        ...
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][unit_id]`, `${vendor.unit_id}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][unit_name]`, `${vendor.unit_name}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][total_price]`, `${Number(vendor.quantity) * Number(vendor.unit_price)}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][profit]`, `${0}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][profit_unit]`, `${vendor.profit_unit}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][fee]`, `${0}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][fee_unit]`, `${vendor.fee_unit}`)
        formData.append(`canvassing_items[${i}][canvassing_vendor][${j}][ongkir]`, `${0}`)
    ...
    tableData.push(
      {
        ...
        max: currency(displayAmount(adjustmentTransactionFeeTotal.value, grandTotal.value)),
        beli: `${adjustmentTransactionFeeTotal.value.type == FeeType.PERCENT ? adjustmentTransactionFeeTotal.value.amount : safePercent(adjustmentTransactionFeeTotal.value.amount, totalBuyingPrice.value)} %`,
        jual: `${adjustmentTransactionFeeTotal.value.type == FeeType.PERCENT ? adjustmentTransactionFeeTotal.value.amount : safePercent(adjustmentTransactionFeeTotal.value.amount, grandTotal.value)} %`,
        min: currency(displayAmount(adjustmentTransactionFeeTotal.value, grandTotal.value)),
        beliMin: `${adjustmentTransactionFeeTotal.value.type == FeeType.PERCENT ? adjustmentTransactionFeeTotal.value.amount : safePercent(adjustmentTransactionFeeTotal.value.amount, totalBuyingPriceMin.value)} %`,
        jualMin: `${adjustmentTransactionFeeTotal.value.type == FeeType.PERCENT ? adjustmentTransactionFeeTotal.value.amount : safePercent(adjustmentTransactionFeeTotal.value.amount, grandTotal.value)} %`,
      },
      {
        label: "Net Profit",
...
const fetchInquiryDetail = async () => {
    ...
    try {
        ...
        const inquiry = await useFetchApi<BaseResponse<Inquiry>>(`/inquiries-read/${inquiry_id.value}`, 'inquiry', 'get', null);
