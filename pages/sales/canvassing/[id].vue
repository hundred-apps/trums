    ...
    >
      <template #header>
        <div class="card-header flex justify-end">
          <el-button
          v-if="canvassingData?.status == CanvassingStatus.DRAFT || canvassingData?.status == CanvassingStatus.CANVASSING"
           type="danger" :icon="Delete" @click="confirmDelete">Hapus</el-button>
          <NuxtLink 
          v-if="canvassingData?.status == CanvassingStatus.DRAFT || canvassingData?.status == CanvassingStatus.CANVASSING"
    ...
    <div v-if="canvassingData?.source">
      <el-card class="my-3">
          ...
          <div class="flex-1">
            <el-descriptions :column="1" size="large" border>
              ...
              <el-descriptions-item label="Permintaan">
                {{ canvassingData?.source?.type === "sales_inquiry" ? 'Sales Inquiry' : "Internal" }}
              </el-descriptions-item>
            </el-descriptions>
          </div>
          <div class="flex-1">
        ...
        <el-descriptions title="Note">
          <el-descriptions-item label="">{{ canvassingData?.source?.description }}</el-descriptions-item>
        </el-descriptions>
      </el-card>
    </div>
    ...
    <el-card class="mb-3">
      ...
      >
        ...
        <el-table-column label="Equivalent Dari" width="300">
          <template #default="{ row }">
            {{ item_canvassing.find((value) =>value.unique_id == row.equivalent_id)?.catalogue_name }}
          </template>
        </el-table-column>
...
import { Delete, Edit, CircleCheck, ArrowDown, Operation } from '@element-plus/icons-vue'
...
import { OperationPriceTag, ReferencePriceTag, VariablePriceTag } from '~/types/pricetag';
...
const selectableCheckbox = (row: CanvassingItemForm, index: number) => {
  return canvassingData.value?.status != CanvassingStatus.PENDING_APPROVAL ? true : false;
}
...
const adjustmentTransactionFeeTotal = computed(() => {
  const filter = canvassingData.value?.reference_transaction?.filter((value) => value.adjustments_transaction?.name.toLowerCase() == 'fee');
  
  if(filter){
    return {...filter[0], amount: filter?.reduce((sum, item) => Number(sum) + Number(item.amount), 0)};
  }
})
const adjustmentTransactionOngkirTotal = computed(() => {
  return canvassingData.value?.reference_transaction?.find((value) => value.adjustments_transaction?.name.toLowerCase() == 'ongkos kirim');
})
  ...
  nextTick(() => {
    ...
    item_canvassing.value.forEach(parent => {
      if (parent.children) {
        ...
        console.log(`Parent ${parent.index}: ${selectedCount}/${parent.children.length} children selected`)
