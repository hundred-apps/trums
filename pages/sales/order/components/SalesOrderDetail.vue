  ...
  <div>
    ...
    >
      <template #header>
        <div class="card-header flex justify-end">
          ...
          <el-button 
            type="success" 
            v-if="purchaseOrderData?.status === PurchaseOrderStatus.PENDING_APPROVAL"
        ...
        <div class="flex-1">
          <el-descriptions title="" :column="1" size="large" border>
            ...
            <el-descriptions-item label="Diskon">
              {{ purchaseOrderData?.is_discount ? `${purchaseOrderData.discount}${purchaseOrderData.discount_unit === 'percent' ? '%' : ''}` : 'Tidak ada' }}
            </el-descriptions-item>
          </el-descriptions>
        </div>
        <div class="flex-1">
          <el-descriptions title="" :column="1" size="large" border>
            <el-descriptions-item label="Status">
              <el-tag :type="getStatusTagType(purchaseOrderData?.status || PurchaseOrderStatus.DRAFT)">
                {{ formatStatus(purchaseOrderData?.status || PurchaseOrderStatus.DRAFT) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="Estimasi Sampai">
              {{ purchaseOrderData?.expected_arrival != null ? formatLocalDate(purchaseOrderData?.expected_arrival) : '-' }}
            </el-descriptions-item>
            
            <el-descriptions-item label="Tanggal Dibuat">
              {{ purchaseOrderData?.created_at != null ? formatLocalDate(purchaseOrderData?.created_at) : '-' }}
            </el-descriptions-item>
      ...
      <el-descriptions title="Informasi Tambahan" v-if="purchaseOrderData?.additional_information">
        <el-descriptions-item label="">{{ purchaseOrderData.additional_information }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-card class="mb-3" shadow="hover">
      ...
      <el-table :data="purchaseOrderData?.purchase_order_item ?? []" border>
        ...
        <el-table-column prop="quantity" label="QTY" align="right" :width="purchaseOrderData?.status === PurchaseOrderStatus.PENDING_APPROVAL ? 200 : 70" >
            <template #default="scope">
                <el-input-number v-model="scope.row.quantity" width="100" :min="1" v-if="scope.row.status === PurchaseOrderItemStatus.PENDING_APPROVAL || (scope.row.status === PurchaseOrderItemStatus.DRAFT && purchaseOrderData?.status === PurchaseOrderStatus.PENDING_APPROVAL)" />
                <p v-else>
                {{ scope.row.quantity }}
                </p>
          </template>
        </el-table-column>
        <el-table-column prop="unit_name" label="UOM" align="center" width="100" />
        <el-table-column prop="unit_price" label="Harga Unit" align="right" width="150">
          <template #default="scope">
            {{ currency(scope.row.unit_price) }}
          </template>
        </el-table-column>
        <el-table-column prop="total_price" label="Total Harga" align="right" width="150">
          <template #default="scope">
        ...
        <el-table-column label="Garansi" width="150">
          <template #default="scope">
            {{ scope.row.is_warranty ? `${scope.row.warranty} ${scope.row.warranty_unit}` : 'Tidak ada' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="Aksi" align="center" width="300" v-if="purchaseOrderData?.status === PurchaseOrderStatus.PENDING_APPROVAL">
          <template #default="scope">
            <el-button 
              type="success" 
              v-if="scope.row.status === PurchaseOrderItemStatus.PENDING_APPROVAL || (scope.row.status === PurchaseOrderItemStatus.DRAFT && purchaseOrderData?.status === PurchaseOrderStatus.PENDING_APPROVAL)"
            ...
            <el-tag v-if="scope.row.status !== PurchaseOrderItemStatus.PENDING_APPROVAL" :type="getItemStatusTagType(scope.row.status)">
    ...
    <el-dialog v-model="visibleApproveDialog" title="Approve Purchase Order!" width="500">
    ...
    <el-dialog v-model="visibleRejectDialog" title="Reject Purchase Order!" width="500">
      <el-form :model="approveForm" :label-position="formApproveLabel">
        <el-form-item label="Catatan" prop="note">
          <el-input v-model="approveForm.note" type="textarea" />
        </el-form-item>
...
const getStatusTagType = (status: string): 'success' | 'info' | 'danger' | 'warning' | 'primary' => {
  switch (status) {
    ...
    case PurchaseOrderStatus.PENDING_APPROVAL: return 'warning'
    case PurchaseOrderStatus.APPROVED: return 'success'
    case PurchaseOrderStatus.CANCELLED: return 'danger'
    case PurchaseOrderStatus.COMPLETED: return 'primary'
...
const getItemStatusTagType = (status: string): 'success' | 'info' | 'danger' | 'warning' | 'primary' => {
  switch (status) {
    case PurchaseOrderItemStatus.DRAFT: return 'info'
    case PurchaseOrderItemStatus.PENDING_APPROVAL: return 'warning'
    case PurchaseOrderItemStatus.DONE: return 'success'
    case PurchaseOrderItemStatus.CANCELLED: return 'danger'
    default: return 'primary'
  }
}
...
const updateStatus = async (status: PurchaseOrderStatus, note: string = '') => {
  ...
  try {
    ...
    (purchaseOrderData.value.purchase_order_item ?? []).forEach((item: PurchaseOrderItem, index: number) => {
...
const rejectItem = async (itemIndex: number) => {
  purchaseOrderData.value!.purchase_order_item[itemIndex].status = PurchaseOrderItemStatus.CANCELLED;
}
...
const subtotal = computed(() => {
  return (purchaseOrderData.value?.purchase_order_item ?? []).reduce((sum, item) => sum + item.quantity * item.unit_price, 0)
})
