  ...
  definePageMeta({
    ...
    const querySearchAsync = (queryString: string, cb: (arg: any) => void) => {
        ...
        useFetchApi<Pagination<ItemSearch[]>>('/catalogues-inventory','fetch-catalogues-inventory', 'post', {
          ...
          if(response.status.value === 'success'){
            ...
            ElMessage.error(response.error.value?.data?.message ?? 'Gagal mengambil data!');
          }
        }).catch((error: any) => {
          ElMessage.error(error.response.data.message);
        });
    ...
    const onHandleSelectItemAutocomplete = async (item: Record<string, any>, scope: any) => {
      if(item.isNew){
        ...
        const catalogue: Catalogue|undefined = await fetchCatalogueDetail(selected.catalogue_id ?? '');
        ...
        <el-table :data="dataTable">
          <el-table-column width="50">
            <template #default="scope">
              <el-button @click="() => openCatalogueDetail(scope.row, scope.$index)" text><el-icon><Warning /></el-icon></el-button>
            </template>
          </el-table-column>
    ...
    <el-drawer v-model="drawerCatalogue" title="Detail Item" :with-header="true">
      ...
      <template #footer>
        <div style="flex: auto">
          ...
          <el-button type="primary" @click="() => handleSubmit(tmpCatalogue!)">Simpan</el-button>
        </div>
      </template>
    </el-drawer>
    </TrumsWrapper>
