<template>
  <TrumsWrapper>
    <el-page-header @back="goBack">
      <template #content>
        <span class="text-large font-600 mr-3">
          Template Checklist - {{ data?.data?.name }}
        </span>
      </template>
    </el-page-header>
    <el-card class="my-3">
      <div class="flex gap-3 my-3">
        <div class="flex-1">
          <el-descriptions title="" :column="1" size="large" border>
            ...
            <el-descriptions-item label="Hour Meter"
              >{{ data?.data.hour_meter }}
            </el-descriptions-item>
            <el-descriptions-item label="Kilo Meter"
    ...
    <el-card class="my-3">
      <el-table
        :data="data?.data.template_checklist_item"
        border
        style="width: 100%"
      >
        <el-table-column
    ...
    <el-card class="my-3">
      <customTable
        :height="500"
        :columns="columnTemplateInventory"
        :data="data?.data.template_checklist_inventory ?? []"
      />
    </el-card>
  </TrumsWrapper>
</template>
...
const { data } = await useFetchApi<BaseResponse<TemplateChecklist>>(
  `/template-checklist-read/${id.value}`,
  "template",
  "get",
  null
);
...
const columnTemplateInventory: Column<TemplateInventories>[] = [
    ...
    headerCellRenderer: () => {
      ...
      const onChange = (value: CheckboxValueType) =>
        (data.value = {
          ...
          success: _data?.success ?? true,
          data: {
            ..._data?.data,
            template_checklist_inventory:
              _data?.data?.template_checklist_inventory.map((row: any) => {
                row.checked = value;
                return row;
      ...
      const allSelected = _data!.data.template_checklist_inventory.every(
        (row) => row.checked
      );
      const containsChecked = _data?.data.template_checklist_inventory.some(
        (row) => row.checked
      );
  ...
  },
  {
    ...
    key: "sn",
    width: 200,
    cellRenderer: ({ rowData }: { rowData: TemplateInventories }) => (
      <>{rowData.inventory?.sn}</>
    ),
  },
  {
    title: "Nama",
    ...
    sortable: true,
    cellRenderer: ({ rowData }: { rowData: TemplateInventories }) => (
      <>{rowData.inventory?.catalogue?.name}</>
    ),
  },

  {
    ...
    width: 200,
    cellRenderer: ({ rowData }: { rowData: TemplateInventories }) => (
      <>{rowData.inventory?.location?.name}</>
    ),
  },
  {
    title: "Operasi",
