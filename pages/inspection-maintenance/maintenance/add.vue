  ...
  <TrumsWrapper>
    ...
    <el-card class="my-3">
      ...
      >
        ...
        <el-form-item v-if="ruleForm.is_repeate" label="Periode Pengulangan" prop="count">
          <el-select-v2
            placeholder="Periode"
      ...
      <el-table :data="ruleForm.jobs">
          <el-table-column prop="name" label="Nama Pekerjaan" >
              <template #default="scope">
                  <el-input v-model="scope.row.name" placeholder="Masukan nama pekerjaan" />
              </template>
          </el-table-column>
          
          <el-table-column prop="status" label="Status">
              <template #default="scope">
                  <el-select v-model="scope.row.status" placeholder="Select" style="width: 240px">
                    <el-option
                      v-for="item in [{
                        value: 'pending',
                        label: 'Pending',
          ...
          <el-table-column prop="unit_name" label="Aksi" width="100">
              <template #default="scope">
                  <el-button type="danger" :disabled="scope.$index == 0" @click="() => removeListJob(scope.$index)" :icon="Delete" circle />
              </template>
          </el-table-column>
      </el-table>
      ...
      <el-table :data="ruleForm.items">
          <el-table-column prop="catalogue_name" label="item" >
              <template #default="scope">
                  ...
                  >
                  <template #append>
                    <el-button @click="() => openDrawerDetailInventory(scope.$index)" :icon="Operation" />
                  </template>
                  </el-autocomplete>
              </template>
          ...
          <el-table-column prop="quantity" label="Quantity">
              <template #default="scope">
                  <el-input v-model="scope.row.quantity" placeholder="Masukan quantity" >
          ...
          <el-table-column prop="unit_name" label="Aksi" width="100">
              <template #default="scope">
                  <el-button type="danger" :disabled="scope.$index == 0" @click="() => removeListItem(scope.$index)" :icon="Delete" circle />
              </template>
          </el-table-column>
      </el-table>
    ...
    <el-drawer v-model="dialog_detail_form_catalogue" :direction="form_catalogue_direction" size="40%">
        ...
        <template #footer>
            <div style="flex: auto">
                ...
                <el-button type="primary" @click="() => onEditInventory(inventoryCreate!)">confirm</el-button>
            </div>
        </template>
    </el-drawer>
  </TrumsWrapper>
...
import { ElMessage, type DrawerProps, type FormInstance, type FormRules } from "element-plus";
import type { Catalogue } from "~/types/catalogue";
import type { Inventory } from "~/types/inventory";
import type { Maintenance, Maintenance_item, Maintenance_job, Team } from "~/types/maintenance";
  ...
  items: [
    {
      ...
      validator: (rule, value: Maintenance_item[], callback) => {
        ...
        const invalid = value.some(item => !item.inventory_id || !item.catalogue_id)
        if (invalid) {
...
const submit = async (formEl: FormInstance | undefined) => {
  ...
  const maintenanceDate = new Date(ruleForm.maintenance_date);
    ...
    const response = await useFetchApi<BaseResponse<any>>('/maintenances-create', 'create-maintenance', 'post', data);
...
const querySearchAsyncPic = (queryString: string, cb: (arg: any) => void) => {
  ...
  const querySearchUnit = (queryString: string, cb: (arg: any) => void) => {
        try {
            ...
            useFetchApi<ResponsePagination<Unit[]>>('/search', 'units', 'post', data).then((response) => {
              if(response.status.value == 'success'){
...
const querySearchAsyncItem = (queryString: string, cb: (arg: any) => void) => {
  ...
  useFetchApi<ResponsePagination<Inventory[]>>("/search", 'search-item-maintenance', 'post', data)
    .then((response) => {
      if (response.status == 200) {
        // console.log(response.data.data.q);
        const inventories: Inventory[] = response.data.value?.data as Inventory[];
...
const onHandleSelectMaintenanceItemAutocomplete = async (record: Record<string, any>, scope: any) => {
      if(record.is_new){
          ...
          try {
              ...
              const response = await useFetchApi<BaseResponse<Inventory>>('/inventories-create', 'create-inventory', 'post', inventoryMap);
              if(response.status.value == 'success'){
                  ...
                  ruleForm.items[scope.$index].inventory_name = dataInventory.catalogue.name;
                  ruleForm.items[scope.$index].catalogue_name = dataInventory.catalogue.name;
...
const querySearchAsyncInventories = (queryString: string, cb: (arg: any) => void, type?: string[]) => {
        const data: RequestSearch = {
            table: 'inventories',
            column: [
                {
        ...
        useFetchApi<ResponsePagination<Inventory[]>>(endpoint, 'inventories', 'post', data).then((response) => {
            if(response.status.value == 'success'){
...
const querySearchLocation = (queryString: string, cb: (arg: any) => void) => {
        ...
        useFetchApi<ResponsePagination<Catalogue[]>>(endpoint, 'location', 'post', data).then((response) => {
            if(response.status.value == 'success'){
                const inventories: Catalogue[] = response.data?.value?.data ?? [];

                if(inventories.length > 0){
                    const results = inventories.map((data: Catalogue) => {
                        return {value: `${data.name}`, unique_id: data.unique_id, object: data};
                    });    
                    cb(results)
                }else{
                    cb([{value: `Tambahkan ${queryString}`, is_new: true, object: {name: queryString}}]);
...
const querySearchPIC = (queryString: string, cb: (arg: any) => void) => {
        ...
        useFetchApi<ResponsePagination<any>>(endpoint, 'people-team', 'post', {keyword: queryString}).then((response) => {
            if(response.status.value == 'success'){
                ...
                if(peopleInTeam.length > 0){
                    const results = peopleInTeam.map((data: PeopleAndTeam) => {
                        return {value: `(${data.type})-${data.name}`, unique_id: data.unique_id, object: data};
                    });    
                    cb(results)
                }
            }
...
const create_catalogue = async (catalogue: Catalogue) => {
    ...
    const onEditInventory = async (inventory: Inventory) => {
      ...
      try {
        ...
        const response = await useFetchApi<BaseResponse<Inventory>>("/inventories-create", "create-inventory", "post", {
        ...
        if (response.status.value == 'success') {
          ...
          ruleForm.items[inventoryCreateIndexed.value].inventory_id = dataInventory.unique_id;
          ruleForm.items[inventoryCreateIndexed.value].catalogue_id = dataInventory.catalogue_id;
          ruleForm.items[inventoryCreateIndexed.value].inventory_name = dataInventory.catalogue.name;
          ruleForm.items[inventoryCreateIndexed.value].catalogue_name = dataInventory.catalogue.name;
          ruleForm.items[inventoryCreateIndexed.value].inventory = dataInventory;
          ruleForm.items[inventoryCreateIndexed.value].catalogue = dataInventory.catalogue;
          ruleForm.items[inventoryCreateIndexed.value].inventory_version = dataInventory.version;
          ...
          ruleForm.items[inventoryCreateIndexed.value].unit_name = dataInventory.unit_name;
          ruleForm.items[inventoryCreateIndexed.value].unit_id = dataInventory.unit_id;
    ...
    const fetchDataEdit = async (id: string) => {
      try {
          const response = await useFetchApi<BaseResponse<Maintenance>>(`/maintenances-read/${id}`,'maintenance-detail','get',null);
          if(response.status.value == 'success'){
            ...
            ruleForm.maintenance_date = new Date(maintenance.maintenance_date * 1000).toDateString() ?? null;
            ruleForm.team_id = "";
            ruleForm.responsible_id = maintenance.responsible_id ?? "";
            ruleForm.start_date = new Date((maintenance.start_date ?? new Date().getTime()) * 1000).toDateString() ?? null;
            ruleForm.end_date = new Date((maintenance.end_date ?? new Date().getTime()) * 1000).toDateString() ?? null;
            ...
            ruleForm.jobs = (maintenance.maintenance_jobs ?? []).map((value) => ({
                ...
                status: value.status,
                pic_id: value.pic_id,
                pic_name: value.is_team ? value.team?.name ?? '' : value.people_pic?.name ?? '',
