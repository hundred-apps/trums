<script lang="tsx" setup>
  definePageMeta({
    ...
    import { InquiryReference, referenceView, TypeInquiry, type Inquiry } from '~/types/inquiry';
...
import FormAddress from '~/components/trums/FormAddress.vue';
    ...
    const maintenances =  await useFetchApi<ResponsePagination<Maintenance[]>>('/search', 'get-maintenance', 'post', request_search_maintenance);
    const sales_order =  await useFetchApi<ResponsePagination<Maintenance[]>>('/search', 'get-sales-order', 'post', request_search_sales_order);
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        item_request.forEach((item, index) => {
          ...
          formData.append(`item_request[${index}][catalogue_id]`, `${item.catalogue_id}`);
          formData.append(`item_request[${index}][catalogue_name]`, `${item.catalogue_name}`);
          formData.append(`item_request[${index}][unit_id]`, `${item.unit_id}`);
          formData.append(`item_request[${index}][unit_name]`, `${item.unit_name}`);
          formData.append(`item_request[${index}][request_qty]`, `${item.request_qty}`);
          formData.append(`item_request[${index}][approved_qty]`, `${item.approved_qty ?? 0}`);
          formData.append(`item_request[${index}][rejected_qty]`, `${item.rejected_qty ?? 0}`);
          formData.append(`item_request[${index}][remaining_qty]`, `${item.request_qty ?? 0}`);
          formData.append(`item_request[${index}][sn]`, `${item.sn}`);
          formData.append(`item_request[${index}][inventory_id]`, `${item.inventory_id}`);
          formData.append(`item_request[${index}][catalogue_version]`, `${item.catalogue_version}`);
        ...
        const response = await useFetchApi<BaseResponse<Inquiry>>('/inquiries-create', 'create-inquiry', 'post', formData);
    ...
    watch(request_search_maintenance, () => {
      refreshNuxtData('get-maintenance');
    }, {immediate: true});
    watch(request_search_sales_order, () => {
    ...
    const onSelectReference_id = async (data: any) => {
      ...
      if(ruleForm.reference == 'non-maintenance'){
        ...
        ruleForm.address_view = generateResultSearchAddress(sales_order.address ?? null).name;
    ...
    const onHandleSelectItemAutocomplete = async (item: Record<string, any>, scope: any) => {
      if(item.isNew){
    ...
    const onHandleSelectContact = async (item: Record<string, any>, type: "to"|"pic") => {
      console.log(item)
      if(item.isNew){
    ...
    const fetchInquiry = async () => {
        ...
        try {
            ...
            if (inquiry.status.value === 'success') {
                ...
                ruleForm.date = inquiryData.date != null ? inquiryData.date! * 1000 : Date.now();
                ...
                ruleForm.address_view = generateResultSearchAddress(inquiryData.address || null).name;
...
<template>
    ...
    <el-card class="my-3">
        ...
        <el-form-item label="reference" prop="reference">
          <el-select v-model="ruleForm.reference" placeholder="Pilih Referensi" style="width: 240px">
            <el-option
              v-for="item in [{value: InquiryReference.MAINTENANCE, label: referenceView(InquiryReference.MAINTENANCE)}, {value: InquiryReference.NON_MAINTENANCE, label: referenceView(InquiryReference.NON_MAINTENANCE)}, {value: InquiryReference.SALES_ORDER, label: referenceView(InquiryReference.SALES_ORDER)}]"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
        ...
        <el-form-item v-if="ruleForm.reference == InquiryReference.MAINTENANCE || ruleForm.reference == InquiryReference.SALES_ORDER" label="Nomor Reference" prop="reference_id">
          <div class="flex gap-2">
            <el-input v-model="ruleForm.reference_view" placeholder="Pilih Nomor Referensi" :disabled="true"/>
            <el-button :icon="Search" type="primary" size="default" @click="showDialog"/>
          </div> 
        </el-form-item>
        ...
        <el-form-item label="Diminta oleh?" prop="to_name" >
            <div class="flex items-center gap-3">
                ...
                />
                <el-button type="primary" v-if="toContact" @click="openDialogTo" :icon="User"/>
            </div>
        </el-form-item>  
        <el-form-item v-if="toContact && toContact.is_company" label="PIC" prop="request_by_name" >
            <div class="flex items-center gap-3">
                <el-autocomplete
                    :fetch-suggestions="querySearchContact"
                ...
                />
                <el-button type="primary" v-if="picContact" @click="openDialogPIC" :icon="User"/>
            </div>
        </el-form-item>  
    ...
    <el-dialog v-model="dialogNewAddress" title="Create New Address" width="500">
      ...
      <el-dialog v-model="dialogSalesOrder" title="Daftar SO" width="1200">
        ...
        <div class="flex justify-end mt-3">
          <el-pagination background layout="prev, pager, next" :total="sales_order?.data.value?.total_data" @next-click="paginationClickSalesOrderClick" @prev-click="paginationClickSalesOrderClick" @change="paginationClickSalesOrderClick" />
        </div>
      </el-dialog>
    </TrumsWrapper>
</template>
