    ...
    <el-page-header @back="goBack">
      <template #content>
        <span class="text-large font-600 mr-3">
          Request - {{ inquiryData?.unique_code }}
        </span>
      </template>
    </el-page-header>
    <div v-if="inquiryData !== undefined">
      <el-card class="my-3">
        <template #header>
          <div class="flex justify-end">
            ...
            <NuxtLink
              :disabled="loading"
              :loading="loading"
              :to="`/inventory-management/inqueiries/add?id=${inquiryData?.unique_id}`"
            ...
            <NuxtLink
              v-if="inquiryData.reference == 'so'"
              :href="`/supply-chain-management/purchase/request/add?inquiry_id=${inquiryData?.unique_id}`"
            ...
            <NuxtLink
              v-if="inquiryData.reference == 'so'"
              :href="`/inventory-management/checkin/add?type=out&inquiry_id=${inquiryData?.unique_id}`"
              class="el-button el-button--primary"
            >
              Check OUT
            </NuxtLink>
            <NuxtLink
              v-if="inquiryData.reference == 'po'"
              :href="`/inventory-management/checkin/add?type=in&inquiry_id=${inquiryData?.unique_id}`"
              class="el-button el-button--primary"
            >
              Check IN
            </NuxtLink>
        ...
        <div class="flex gap-3 my-3">
          ...
          <div class="flex-1">
            <el-descriptions title="" :column="1" size="small" border>
              ...
              <el-descriptions-item label="Referensi"
                >{{ referenceView(inquiryData?.reference as InquiryReference ?? InquiryReference.MAINTENANCE) }}
              </el-descriptions-item>
              <el-descriptions-item label="Nomor Referensi">
                <NuxtLink
                  class="text-blue-600"
                  :href="`${getUrlReference(inquiryData?.reference as InquiryReference ?? InquiryReference.MAINTENANCE, inquiryData.reference_data?.unique_id ?? '')}`"
                  >{{ inquiryData?.reference_data?.unique_code }}</NuxtLink
                >
              </el-descriptions-item>
            </el-descriptions>
          </div>
      ...
      <el-card>
        ...
        >
          ...
          <!-- <el-table-column label="PR" width="200">
            <template #default="scope">
              <component
                :is="getReferenceItem(scope.row, ItemRequestTrailReference.PR)"
              />
            </template>
          </el-table-column>
          <el-table-column label="PO" width="200">
            <template #default="scope">
              <component
                :is="getReferenceItem(scope.row, ItemRequestTrailReference.PO)"
              />
            </template>
          </el-table-column> -->
...
const getReferenceItem = (
  ...
  findReference: ItemRequestTrailReference
): JSX.Element => {
  if (item_request.item_request_trail.length > 0) {
    const find: ItemRequestTrail | undefined =
      item_request.item_request_trail.find(
        (trail) => trail.reference === findReference.toLowerCase()
      );
    ...
    if (!find) {
      ...
      return (
        <NuxtLink
          href={`/supply-chain-management/purchase/request/${
            find!.data_reference?.unique_id
          }`}
        >{`${find!.data_reference?.unique_code}`}</NuxtLink>
      );
    }
  } else {
    return <p>N/A</p>;
...
const getUrlReference = (ref_data: InquiryReference, ref_id: string) => {
  switch (ref_data) {
    case InquiryReference.MAINTENANCE:
      return `/inspection-maintenance/maintenance/${ref_id}`;
    case InquiryReference.PURCHASE_ORDER:
      return `/supply-chain-management/purchase/order/${ref_id}`;
    case InquiryReference.SALES_ORDER:
      return `/sales/order/${ref_id}`;
