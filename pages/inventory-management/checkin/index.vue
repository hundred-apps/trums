<script lang="tsx" setup>
  ...
  const fetchData = async () => {
    loading.value = true;
    try {
      const response = await useFetchApi<ResponsePagination<InventoryMovement[]>>(`/search`, 'inventory_movement', 'post', request_search.value);

  
      if(response.status.value == 'success'){
        data.value = response.data.value as ResponsePagination<InventoryMovement[]>;
      }
    } catch (error: any) {
  ...
  const {data} = await useFetchApi<ResponsePagination<InventoryMovement[]>>(`/search`, 'inventory_movement', 'post', request_search.value);
  ...
  const handleDelete = async (row: string[]) => {
    console.log("Editing:", row);
    try{
      const response = await useFetchApi('/inventory-movement-delete', 'inventory_movement', 'delete', row);
      if(response.status.value == 'success'){
