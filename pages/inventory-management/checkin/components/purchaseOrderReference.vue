  ...
  <el-row :gutter="20" class="mb-3">
    <el-col :span="6">
      ...
      <el-button
        ...
        :disabled="
          (data?.data ?? []).filter((value) => value.checked).length === 0
        "
      >
        Tambahkan ({{
          (data?.data ?? []).filter((value) => value.checked).length
        }})
      </el-button>
      <el-button
        size="default"
        type="default"
        @click="() => refreshNuxtData('get-purchase-order-item')"
      >
        Reload
      </el-button>
    </el-col>
...
const { data } = await useFetchApi<ResponsePagination<PurchaseOrderItem[]>>(
  "/search",
