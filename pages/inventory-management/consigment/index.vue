...
import { InfoFilled } from '@element-plus/icons-vue'
  ...
  const createConsigment = () => router.push('/inventory-management/consigment/add');

  const fetchData = async () => {
    loading.value = true;
    try {
      const response = await useFetchApi<ResponsePagination<Consigment[]>>(`/search`, 'transaction_recaps', 'post', request_search.value);
  ...
  const {data} = await useFetchApi<ResponsePagination<Consigment[]>>(`/search`, 'transaction_recaps', 'post', request_search.value);
  ...
  const availableColumn: Column<Consigment>[] = [
    ...
    },
    {
      ...
      key: 'unique_code',
      width: 200,
      cellRenderer: ({rowData: row}) => <NuxtLink href={`checkin/${row.unique_id}`} class={'text-blue-600'}>{row.unique_code}</NuxtLink>
    },
    {
      title: 'Nama', 
    ...
    },
    {
      ...
      key: '',
      width: 10,
      cellRenderer: ({rowData: row}) => <p>{row.transaction_recaps_item.length}</p>
    },
    {
      title: 'Total', 
      dataKey: '',
  ...
  const getTotalTransaction = (data: Consigment) => {
    ...
    return currency(data?.transaction_recaps_item.reduce((prev: any, curr: any) => prev + (curr.quantity * curr.selling_price), 0))
  }
  ...
  const deleteBulk = async (ids: string[]) => {
    ...
    loading.value = true;
    try {
      const response = await useFetchApi('/transaction-recaps-delete', 'transaction_recaps', 'post', ids);
      if(response.status.value == 'success'){
...
<template>
    <TrumsWrapper>
      <el-row :gutter="20" class="mb-3">
        ...
        <el-button size="default" @click="createConsigment" >Consignment</el-button>
        <el-popconfirm
        ...
        >
          ...
          <template #actions="{ confirm, cancel }">
            ...
            <el-button
              type="danger"
              size="small"
              @click="() => deleteBulk(selectionItem.map((value) => (value.unique_id)))"
            >
              Hapus
            </el-button>
          </template>
      ...
      <el-table v-if="!loading" :data="data?.data" style="width: 100%" @selection-change="handleSelectionChange">
          <el-table-column type="selection" :selectable="(data, index) => true" width="50" />
