<template>
  <TrumsWrapper>
    ...
    <el-button
      type="primary"
      :icon="Plus"
      @click="() => (visibleModalContact = true)"
      >Tambah Vendor</el-button
    >
        ...
        <el-table-column prop="unit_name" label="UOM" align="center">
          <template #default="scope">
            <el-autocomplete
              ...
              @select="
                (item) => handleSelectUnit(item, itemIndex, scope.row.index)
              "
            />
        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              circle
              @click="
                () => openModalPrice(itemIndex, scope.row.index, 'search')
              "
            />
            <el-button
              v-if="scope.row.type == 'parent'"
              ...
              circle
              @click="
                () => openModalPrice(itemIndex, scope.row.index, 'create')
              "
            />
          </template>
        </el-table-column>
      ...
      <div class="mt-4 border-t pt-4 space-y-3">
        ...
        <div class="flex items-center gap-3">
          ...
          >
            <template #append>
              <el-select
                v-model="summeries[itemIndex].discount_unit"
                placeholder="Select"
                style="width: 115px"
              >
    ...
    >
      <el-row :gutter="20" class="mb-3">
        ...
        <el-button
          ...
          @click="
            addToForm(
              (itemRequest?.data ?? []).filter(
                (i: ItemRequestTrail) => i.checked
              )
            )
          "
    ...
    <el-dialog
      v-model="visibleModalPricetagItem"
      title="Cari Penawaran"
      width="1000"
    >
      <el-row :gutter="20" class="mb-3">
        <el-col :span="6"
      ...
      <el-table
        ...
        style="width: 100%"
        @selection-change="handleSelectionChangePriceTagItem"
      >
        <el-table-column type="selection" width="55" />
        <el-table-column label="Vendor">
          <template #default="scope">
            <p>
              {{
                scope.row.pricetag.owner_id == null
                  ? "-"
        ...
        <el-table-column label="Nama Item" width="180">
          <template #default="scope">
            <NuxtLink
              class="text-blue-500"
              :href="`/catalogue/${scope.row.unique_id}`"
              >{{ scope.row.catalogue.name ?? "" }}</NuxtLink
            >
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="visibleModalContact" title="Cari Vendor" width="1000">
      ...
      <div class="flex justify-end mt-3">
        <el-pagination
          ...
          layout="prev, pager, next"
          :total="itemRequest?.total_data"
          @next-click="paginationClickContact"
          @prev-click="paginationClickContact"
          @change="paginationClickContact"
        />
      </div>
    </el-dialog>
    <el-dialog
...
}> = ({ value, onChange, onBlur, onKeydownEnter, forwardRef }) => {
  return (
    <ElInputNumber
      ref={forwardRef as any}
      onUpdate:modelValue={onChange}
...
const querySearchContact = (
  ...
  try {
    ...
    useFetchApi<ResponsePagination<Contact[]>>(
      ...
      if (response.status.value == "success") {
        const contacs: Contact[] = (response.data.value?.data ??
          []) as Contact[];
        if (contacs.length > 0) {
          cb(
            contacs.map((value) => ({
...
const openModalPrice = (
  ...
  const findItems = (list: any[], indexKey: string) => {
    ...
    if (dataIndex !== -1) {
      ...
      request_search_pricetag_item.value.owner_id =
        vendors.value[vendorIndex].vendor_id;
      return true;
    }
    ...
    if (deleted) {
      ...
      vendors.value.forEach((vendor) => {
        vendor.items = vendor.items.filter(
          (value) => value.item_request_trail_id !== item.item_request_trail_id
        );
      });
    }
  } else {
...
const addNewVendorOffer = (itemIndex: number) => {
  ...
  ruleForm.canvassing_item[itemIndex].children.push({
    index: `${itemIndex}-${
      ruleForm.canvassing_item[itemIndex].children.length + 1
    }`,
    unique_id: "",
    offer_item_id: "",
    vendor_id: "",
...
const addToOfferVendor = (val: Pricetag_item[]) => {
  for (let index = 0; index < val.length; index++) {
    const element = val[index];
    const vendor_items =
      vendors.value[offerIndex.value].items[offerItemIndex.value];
...
const removeVendorOffer = async (vendorIndex: number, index: string) => {
  const findAndRemove = async (list: any[], indexKey: string) => {
    ...
    if (dataIndex !== -1) {
      if (
        list[dataIndex].unique_id !== "" &&
        list[dataIndex].unique_id !== null
      ) {
...
const onHandleSelectVendor = (
  ...
  if (item.isNew) {
    createNewVendor(item.query).then((customer) => {
      if (customer) {
        ruleForm.canvassing_item[request_index].children[
          vendor_index
    ...
    ruleForm.canvassing_item[request_index].children[vendor_index].vendor_id =
      customer.unique_id;
    ruleForm.canvassing_item[request_index].children[vendor_index].vendor_name =
      customer.name;
  }
};

const querySearchCatalogues = (
  query: string,
  cb: (arg: any) => void,
  itemIndex: number,
  vendorIndex: number
      ...
      column: [
        {
          offer: {
            contact_id:
              ruleForm.canvassing_item[itemIndex].children[vendorIndex]
                .vendor_id,
          },
        },
      ],
...
const handleSelectCatalogue = (
  ...
  ruleForm.canvassing_item[itemIndex].children[vendorIndex].catalogue_name =
    selectedCatalogue.name;
  ruleForm.canvassing_item[itemIndex].children[vendorIndex].catalogue_id =
...
const submit = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    ruleForm.canvassing_item.forEach((value, i) => {
      ...
      formData.append(
        `canvassing_items[${i}][item_request_trail_id]`,
        `${value.item_request_trail_id}`
      );
      formData.append(
        `canvassing_items[${i}][canvassing_version]`,
        `${value.canvaasing_version}`
      );
      formData.append(
        `canvassing_items[${i}][item_request_trail_version]`,
        `${value.item_request_trail_version}`
      );
      formData.append(`canvassing_items[${i}][unit_id]`, `${value.unit_id}`);
      formData.append(
        `canvassing_items[${i}][unit_name]`,
        `${value.unit_name}`
      );
      formData.append(
        `canvassing_items[${i}][unit_version]`,
        `${value.unit_version}`
      );
...
const getReference = (data: Inquiry) => {
  ...
  if (data.reference == "internal") {
    const contact: Contact | null = (data?.reference_data ??
      null) as Contact | null;
    return contact == null ? <div>-</div> : <div>{contact?.name}</div>;
  } else {
    const maintenance: Maintenance | null = (data?.reference_data ??
...
const onSort = (sortBy: SortBy) => {
  ...
  data.sort = {
    ...
    order:
      request_search_inquiry.value.sort?.order == OrderColumn.ASC
        ? OrderColumn.DESC
        : OrderColumn.ASC,
  };
...
const handleSelectionChange = (val: ItemRequestTrail[]) => {
  (itemRequest.value?.data ?? []).forEach((value) => {
    if (
      val.find((data) => data.unique_id == value.unique_id)?.unique_id ==
      value.unique_id
    ) {
      value.checked = true;
    } else {
...
const fetchInquiry = async () => {
  loading.value = true;
  try {
    const response = await useFetchApi<ResponsePagination<ItemRequestTrail[]>>(
      "/search",
      "search-request",
      "post",
      request_search_inquiry.value
    ...
    if (response.status.value == "success") {
      ...
      if (canvasing != null) {
        ...
        ruleForm.canvassing_item = canvasing.canvassing_item.map((value) => ({
          ...
          offer_item_version: 0,
          catalogue_name:
            value.item_request_trail?.item_request?.catalogue_name ?? "",
          catalogue_id:
            value.item_request_trail?.item_request?.catalogue_id ?? "",
...
const handleSelectUnit = (
  ...
  const itemExist = vendors.value[vendorIndex].items.find(
...
function groupByVendor(
  canvassingItems: CanvassingItem[]
): CanvasingVendorView[] {
  ...
  canvassingItems.forEach((item: CanvassingItem, itemIndex: number) => {
    ...
    const vendorIds = [
      ...new Set(item.canvassing_vendor.map((v) => v.vendor_id)),
    ];

    vendorIds.forEach((vendorId) => {
      const vendorObj = item.canvassing_vendor.find(
      ...
      const alreadyExists = groupedVendor.items.some(
        (i) => i.unique_id === item.unique_id
      );

      if (!alreadyExists) {
        const itemFind = item.canvassing_vendor.filter(
          (v) => v.vendor_id === vendorId
        );
        ...
        groupedVendor.items.push({
          ...
          offer_item_version: 0,
          catalogue_id:
            item.item_request_trail?.item_request?.catalogue_id ?? "",
          catalogue_name:
            item.item_request_trail?.item_request?.catalogue_name ?? "",
