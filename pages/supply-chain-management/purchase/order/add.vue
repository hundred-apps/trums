<template>
  <TrumsWrapper>
    ...
    <el-card class="mb-3">
      ...
      <el-table :data="ruleForm.items" border style="width: 100%">
        ...
        <el-table-column label="Nomor PR" width="200">
          <template #default="scope">
            <el-input
              v-if="!scope.row.pr_item_request_trail_id"
            ...
            >
              <template #append style="background-color:blue !important">
                <el-button :icon="Search"  type="primary" @click="() => openModalPr(scope.$index)" />
              </template>
            </el-input>
            <div v-else>{{ scope.row.pr_number }}</div>
          </template>
        </el-table-column>
    ...
    <!-- Modals -->
    <el-dialog v-model="visibleCanvassingModal" title="Pilih Item dari Canvassing" width="1000">
      ...
      >
        ...
        <el-table-column prop="unit_name" label="Satuan" width="100" />
        <el-table-column prop="unit_price" label="Harga Satuan" width="120" align="right">
          <template #default="scope">
            {{ formatCurrency(scope.row.unit_price) }}
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="visibleModalRequest" title="Cari Permintaan" width="1000">
        ...
        <el-table :data="itemRequest?.data.value?.data ?? []" style="width: 100%">
            ...
            <el-table-column label="Aksi" >
                <template #default="scope">
                    <el-button size="small" type="primary" @click="() => selectPR(scope.row)">Pilih</el-button
                    >
                </template>
            </el-table-column>
...
const pricetagItems = await useFetchApi<ResponsePagination<Pricetag_item[]>>('/search', 'search-pricetag-item', 'post', query_search_pricetag_item.value);
...
const itemRequest = await useFetchApi<ResponsePagination<ItemRequestTrail[]>>('/search', 'search-item-request', 'post', request_search_inquiry.value);
...
// Computed
const filteredCanvassingItems = await useFetchApi<ResponsePagination<CanvassingVendor[]>>('/search', 'search-canvasing-vendor', 'post', query_search_canvasing_item.value);



const subtotal = computed(() => {
  return ruleForm.items.reduce((sum, item) => sum + (item.unit_price * item.quantity), 0)
})
...
const selectPR = (row: ItemRequestTrail) => {
  ...
  ruleForm.items[poItemIndex.value].pr_item_request_trail_version = row.version ??1;
...
const deleteItem = async (ids: string[]) => {
  try {
    const response = await useFetchApi<BaseResponse<any>>('/po-item-delete', 'delete-po-item', 'post', ids);
    if(response.status.value === 'success'){
    ...
    if (valid) {
      ...
      try {
        ...
        ruleForm.items.forEach((value, index) => {
          ...
          formData.append(`item[${index}][item_request_id]`, `${value.item_request_trail?.item_request_id}`);
          formData.append(`item[${index}][item_request_version]`, `${value.item_request_trail?.item_request_version}`);
          formData.append(`item[${index}][pr_item_request_trail_id]`, `${value.pr_item_request_trail_id}`)
          formData.append(`item[${index}][pr_item_request_trail_version]`, `${value.item_request_trail_version}`)
          formData.append(`item[${index}][item_request_trail_id]`, `${value.item_request_trail_id}`)
          formData.append(`item[${index}][item_request_trail_version]`, `${value.item_request_trail_version}`)
        ...
        if (response.status.value === 'success') {
          ...
          if(purchaseOrder){
            router.push(`/supply-chain-management/purchase/order/${purchaseOrder.unique_id}`);
          }
        }
