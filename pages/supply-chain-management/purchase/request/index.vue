<template>
  <TrumsWrapper>
    <el-row :gutter="16">
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).filter(item => item.status === 'draft').length">
            <template #title>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).filter(item => item.status === 'waiting_approval').length">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                Waiting Approval
              </div>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).filter(item => item.status === 'approved').length">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                Approved
              </div>
      ...
      <el-col :span="6">
        <div class="statistic-card">
          <el-statistic :value="(data?.data ?? []).filter(item => item.status === 'canvassing').length">
            <template #title>
              <div style="display: inline-flex; align-items: center">
                In Canvassing
              </div>
    ...
    <el-row :gutter="20" class="mb-3">
      ...
      <NuxtLink class="el-button el-button--primary el-button--default" href="/supply-chain-management/purchase/request/add">
      ...
      >
        ...
        <el-form-item label="Status" prop="status">
          <el-select v-model="ruleFormFilter.status" multiple
            collapse-tags placeholder="Select Status" style="width: 100%" :loading="loadingFilter">
            <el-option
...
import { type Column, type CheckboxValueType, TableV2FixedDir, ElPopover, ElCheckbox, ElIcon, type SortBy, ElCheckboxGroup, type FormInstance, type FormProps } from 'element-plus'
import type { PurchaseRequest, PurchaseRequestStatus } from '~/types/purchase_request'
...
// Data state
const {data} = await useFetchApi<ResponsePagination<PurchaseRequest[]>>(`/search`, 'purchase-requests', 'post', request_search.value);
...
// Columns
const columnsSelected = ref<string[]>(['selection', 'unique_code', 'source_document', 'requester_name', 'created_at', 'status', 'operations', 'setup'])
const columns: Column<PurchaseRequest>[] = [
  {
    ...
    cellRenderer: ({ rowData: row }) => (
      <NuxtLink href={`/supply-chain-management/purchase/request/${row.unique_id}`} class="text-blue-500">
        {row.unique_code}
      </NuxtLink>
    )
  },
              ...
              <div class="filter-wrapper">
                <div class="filter-group flex flex-col">
                  <ElCheckboxGroup v-model={request_search.value.column[0].status}>
                    ...
                    <ElCheckbox key="waiting_approval" value="waiting_approval" label="Waiting Approval" />
                    <ElCheckbox key="approved" value="approved" label="Approved" />
                    <ElCheckbox key="rejected" value="rejected" label="Rejected" />
                    <ElCheckbox key="canvassing" value="canvassing" label="Canvassing" />
                  </ElCheckboxGroup>
                </div>
              </div>
            ),
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }: { rowData: PurchaseRequest }) => (
      <>
         <NuxtLink class="el-button el-button--small" href={`/supply-chain-management/purchase/request/add?id=${rowData.unique_id}`} >Edit</NuxtLink>
...
columns.unshift({
  ...
  cellRenderer: ({ rowData }) => {
    ...
    return <SelectionCell value={false} onChange={onChange} />
  },
  headerCellRenderer: () => {
...
const onEdit = (purchaseRequest: PurchaseRequest) => {
  router.push(`/supply-chain/purchase-request/edit/${purchaseRequest.unique_id}`)
}
...
const onDelete = async (uniques: string[]) => {
  try {
    ...
    const response = await useFetchApi<BaseResponse<any>>('/purchase-request-delete', 'delete-purchase-request', 'post', uniques);
...
const remoteSearchRequester = (query: string) => {
  ...
  try {
    ...
    useFetchApi<ResponsePagination<Contact>>('/search', 'search-requester', 'post', request_contact).then((response) => {
      if(response.status.value == 'success'){
...
const fetchData = async () => {
  loading.value = true;
  try {
    const response = await useFetchApi<ResponsePagination<PurchaseRequest[]>>(`/search`, 'purchase-requests', 'post', request_search.value);
    if(response.status.value == 'success'){
