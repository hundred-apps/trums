  ...
  <TrumsWrapper>
    ...
    <el-card class="mb-3">
      ...
      <el-table :data="ruleForm.items" border>
        ...
        <el-table-column prop="request_number" label="Nomor Permintaan" width="200"/>
        <el-table-column prop="quantity" label="Request QTY" align="right" />
        <el-table-column prop="request_quantity" label="QTY Purchase" align="right">
      ...
      <el-button class="mt-4" style="width: 100%" @click="visibleModalItemRequest = true">
        Tambah Item
      </el-button>
    </el-card>
    
    <!-- Modal untuk mencari Item Request -->
    <el-dialog v-model="visibleModalItemRequest" title="Cari Item Request" width="1000">
...
import { ItemRequestTrailReference, ItemRequestTrailStatus, type ItemRequest, type ItemRequestTrail } from '~/types/item_request'
...
// Form data
const ruleForm = reactive<Omit<PurchaseRequest, 'created_at' | 'updated_at' | 'version'> & {
...
const getItemRequestStatusType = (status: string): 'success' | 'info' | 'danger' | 'warning' | 'primary' => {
  const statusTypes: Record<string, 'success' | 'info' | 'danger' | 'warning' | 'primary'> = {
    'draft': 'info',
...
const querySearchContact = (queryString: string, cb: (arg: any) => void) => {
  ...
  useFetchApi<ResponsePagination<Contact[]>>('/search','search-contact', 'post', request_search).then((response) => {
    if(response.status.value == 'success'){
...
const submitPurchaseRequest = async () => {
  try {
    ...
    if (response.status.value === 'success') {
      ...
      router.push(`/supply-chain-management/purchase/request/${response.data.value?.data.unique_id}`);
    }
  } catch (error: any) {
    ElMessage.error(error.response?.data?.message || 'Gagal membuat purchase request')
    throw error
  }
}
