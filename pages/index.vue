...
const fetchData = async () => {
  try {
    ...
    posts.value = response.data.data; // Batasi ke 10 data untuk demo
  } catch (error) {
...
// Fungsi menyimpan data (POST/PUT)
const saveData = async () => {
  try {
    if (isEditMode.value) {
      ...
      // Add data
      const newUser = { name: form.value.name }; // Kirim hanya name
