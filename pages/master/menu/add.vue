...
import { ApprovalPermissionStatus, ApprovalType, PermissionType, type ApprovalPermissionPIC, type Menu, type Permission } from '~/types/menu';
...
const rules = reactive<FormRules<MenuForm>>({
  ...
  route: [
    ...
    {
      pattern: /^\/[a-zA-Z0-9\-_\/]*$/,
      message: 'Route harus dimulai dengan / dan hanya boleh mengandung huruf, angka, dash, underscore, dan slash',
      trigger: 'blur',
    }
  ],
  order: [
...
const pageTitle = computed(() => 
  isEditMode.value ? 'Edit Menu' : parent_id.value ? 'Tambah Submenu' : 'Tambah Menu Baru'
)
...
const fetchParentMenus = async () => {
  try {
    const response = await useFetchApi<ResponsePagination<Menu[]>>('/search', 'get-parent-menus', 'post', request_search);
...
const fetchMenuDetail = async () => {
  ...
  loading.value = true;
  try {
    const response = await useFetchApi<BaseResponse<Menu>>(`/menu-read/${id.value}`, 'get-menu-detail', 'get', null);
    if (response.status.value === 'success') {
  ...
  try {
    ...
    const payload = {
      ...
      permission: ruleForm.permissions.map((permission) => ({
        ...
        approval_permission_pic: (permission.approval_permission_pic ?? []).map((value) => ({
    ...
    let response = await useFetchApi<BaseResponse<Menu>>('/menu-create', 'create-menu', 'post', payload);

    if (response.status.value === 'success') {
      ElMessage.success(isEditMode.value ? 'Menu berhasil diperbarui' : 'Menu berhasil dibuat');
      if(!id.value){
...
const querySearchParentMenus = (queryString: string, cb: (arg: any) => void) => {
...
if (parent_id.value) {
  ...
  onMounted(async () => {
    ...
    const parentMenu = parentMenus.value.find(menu => menu.unique_id === parent_id.value);
...
onMounted(async () => {
  ...
  if (id.value) {
    ...
    const maxOrder = Math.max(...parentMenus.value.map(menu => menu.order || 0), 0);
...
const querySearchAsyncPIC = (queryString: string, cb: (arg: any) => void) => {
  ...
  useFetchApi<ResponsePagination<People[]>>('/search', 'search-people','post', search_request).then((response) => {
    if(response.status.value == 'success'){
      const resultApi: People[] = response.data.value?.data || [];
      if(resultApi.length > 0){
        const results = resultApi.map((data: People) => {
            return {...data, value: `${data.name} ${data.email ? ' - ' + data.phone : ''}`}
        });    
        cb(results)
      }else{
...
const querySearchAsyncDept = (queryString: string, cb: (arg: any) => void) => {
  ...
  useFetchApi<ResponsePagination<Departement[]>>('/search', 'search-dept','post', search_request).then((response) => {
    if(response.status.value == 'success'){
...
const querySearchAsyncPosition = (queryString: string, cb: (arg: any) => void) => {
  const search_request: RequestSearch = {
    keyword: queryString,
    table: 'positions',
    column: [],
  ...
  useFetchApi<ResponsePagination<Position[]>>('/search', 'search-position','post', search_request).then((response) => {
    if(response.status.value == 'success'){
...
const onHandleSelectPIC = async (item: Record<string, any>, index: number, scope: any) => {
  if(item.isNew){
    ...
    if(selected != null){
      ruleForm.permissions[index].approval_permission_pic![scope.$index].pic = selected;
      ruleForm.permissions[index].approval_permission_pic![scope.$index].pic_id = selected.unique_id;
    }else{
...
const onHandleSelectDept = async (item: Record<string, any>, index: number, scope: any) => {
  const selected: Departement = item as Departement;
  ruleForm.permissions[index].approval_permission_pic![scope.$index].departement = selected;
  ruleForm.permissions[index].approval_permission_pic![scope.$index].departement_id = selected.unique_id;
  
}
const onHandleSelectPosition = async (item: Record<string, any>, index: number, scope: any) => {
  const selected: Position = item as Position;
  ruleForm.permissions[index].approval_permission_pic![scope.$index].position = selected;
  ruleForm.permissions[index].approval_permission_pic![scope.$index].position_id = selected.unique_id;
...
<template>
  <TrumsWrapper>
        ...
        <el-card v-if="ruleForm.name" class="mt-4" shadow="never">
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-mono text-blue-600">{{ ruleForm.route || '-' }}</span>
            </div>
            <div class="flex justify-between">
        ...
        <el-table :data="ruleForm.permissions" width="100%" border>
          <el-table-column label="Nama">
            <template #default="scope">
              <el-input v-model="scope.row.permission_name" placeholder="Nama Permission" />
            </template>
          </el-table-column>
          <el-table-column width="200" label="Tipe Permission">
            <template #default="scope">
              <el-select v-model="scope.row.type" placeholder="Select" @change="(val) => initialApprovalSetting()" >
                <el-option
                    v-for="item in [{value: PermissionType.APPROVAL, label: 'Approval'}, {value: PermissionType.PERMISSION, label: 'Basic'}]"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                />
          ...
          <el-table-column width="150" label="Aturan Approval" prop="approval_type">
            <template #default="scope">
              <el-select v-model="scope.row.approval_type" placeholder="Select" >
                <el-option
                    v-for="item in [{value: ApprovalType.PARALLEL, label: 'Parallel'}, {value: ApprovalType.SEQUENTIAL, label: 'Sequential'}]"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                />
          ...
          <el-table-column width="150" label="Tingkatan Approval" prop="approval_length">
            <template #default="scope">
                <el-input-number @change="initialApprovalSetting" v-model="scope.row.approval_length" />
            </template>
          </el-table-column>
    ...
    <div v-for="approval_permission, index in ruleForm.permissions">
      <el-card v-if="approval_permission.type == PermissionType.APPROVAL" class="my-3" >
          <template #header>
            <div class="card-header">
              {{`${approval_permission.approval_type} ${approval_permission.permission_name} ${approval_permission.approval_length} step`}}
            </div>
          </template>
          <el-table :data="approval_permission.approval_permission_pic" width="100%" border>
