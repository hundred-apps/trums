<template>
  <TrumsWrapper>
    <el-row :gutter="20" class="mb-3">
      ...
      <NuxtLink class="el-button el-button--primary el-button--default" href="/contact-management/contacts/form/add?mode=add&company=false&ownership=true">
...
// Data state
const { data } = await useFetchApi<ResponsePagination<Contact[]>>(`/search`, 'contact', 'post', request_search.value);
...
// Columns
const columnsSelected = ref<string[]>(['selection', 'unique_code', 'name', 'email', 'phone', 'is_personal', 'created_at', 'operations', 'setup'])
const columns: Column<Contact>[] = [
  {
    ...
    cellRenderer: ({ rowData: row }) => (
      <NuxtLink href={`/contact-management/contacts/${row.unique_id}`} class="text-blue-500">
        {row.unique_code}
      </NuxtLink>
    )
  },
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }: { rowData: Contact }) => (
      <ElTag type={rowData.is_personal ? 'success' : 'primary'}>
        {rowData.is_personal ? 'Personal' : (rowData.is_company ? 'Perusahaan' : 'Lainnya')}
      </ElTag>
    )
  },
  {
...
const onDelete = async (contactIds: string[]) => {
  try {
    ...
    const response = await useFetchApi<BaseResponse<any>>('/contact-delete', 'delete-contact', 'post', contactIds);
...
const fetchData = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.message || 'Gagal memuat data kontak')
  } finally {
    loading.value = false
  }
}
