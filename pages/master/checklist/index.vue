<template>
    <TrumsWrapper>
        <el-row :gutter="20" class="mb-3">
            <el-col :span="6"
                ><el-input v-model="request_search.keyword" size="default" placeholder="Type to search"
            /></el-col>
            <NuxtLink href="checklist/add" @click="() => {
              const checklist_id = useCookie('checklist_id');
              checklist_id.value = null;
            }" class="el-button el-button--default" >Buat Checklist</NuxtLink>
            <el-button
                size="default"
                @click="fetchData"
                :loading-icon="Eleme"
...
import { ElButton, ElCheckbox, ElIcon, ElPopover, type CheckboxValueType, type Column, type SortBy } from 'element-plus';
...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await useFetchApi<ResponsePagination<ChecklistAttribute[]>>(`/search`, 'maintenance_checklist', 'post', request_search.value);


        if(response.status.value == 'success'){
            data.value = response.data.value as ResponsePagination<ChecklistAttribute[]>;
        }
    } catch (error: any) {
...
const {data} = await useFetchApi<ResponsePagination<ChecklistAttribute[]>>(`/search`, 'maintenance_checklist', 'post', request_search.value);
...
const availableColumn: Column<ChecklistAttribute>[] = [
    ...
    },
    {
      ...
      width: 300,
      sortable: true,
      cellRenderer: ({rowData: row}) => <NuxtLink href={`checkin/${row.unique_id}`} class={'text-blue-600'}>{row.name}</NuxtLink>
    },
    {
      title: 'Tipe', 
      dataKey: 'value_type',
              ...
              default: () => (
                <div class="filter-wrapper">
                  <div class="filter-group flex flex-col">
                    <ElCheckbox value="text" v-model={request_search.value.column![0].value}>
                      Text
                    </ElCheckbox>
                    <ElCheckbox value="option" v-model={request_search.value.column![0].value}>
                      Option
                    </ElCheckbox>
                    <ElCheckbox value="select" v-model={request_search.value.column![0].value}>
                      Select
                    </ElCheckbox>
                    <ElCheckbox value="multi_select" v-model={request_search.value.column![0].value}>
                      Multi Select
                    </ElCheckbox>
                    <ElCheckbox value="file" v-model={request_search.value.column![0].value}>
                      File
                    </ElCheckbox>
                  </div>
                </div>
...
const handleDelete = async (ids: string[]) => {
    ...
    try {
        ...
        const response = await useFetchApi<BaseResponse<any>>(`/maintenance-checklist-delete`, 'maintenance-checklist-delete', 'post', ids);
        if(response.status.value == 'success'){
