...
import { type Column, type CheckboxValueType, TableV2FixedDir, ElPopover, ElCheckbox, ElIcon, type SortBy, ElCheckboxGroup, type FormInstance, type FormProps, ElMessage, ElMessageBox } from 'element-plus'
...
// Data state
const { data } = await useFetchApi<ResponsePagination<Tax[]>>('/search', 'tax', 'post', request_search.value);
...
// Columns
const columnsSelected = ref<string[]>(['selection', 'tax_name', 'tax_value', 'created_at', 'operations', 'setup'])
const columns: Column<Tax>[] = [
  ...
  },
  {
    ...
    sortable: true,
    cellRenderer: ({ rowData }: { rowData: Tax }) => (
      <span>{rowData.created_at ? formatLocalDate(rowData.created_at) : '-'}</span>
    )
  },
  
  {
    ...
    cellRenderer: ({ rowData }: { rowData: Tax }) => (
      <>
        <NuxtLink class="el-button el-button--small" href={`/master/tax/add?id=${rowData.unique_id}`}>Edit</NuxtLink>
...
const onDelete = async (id: string[]) => {
  ...
  try {
    ...
    const response = await useFetchApi<BaseResponse<any>>('/taxs-delete', 'delete-tax', 'post', id);
...
const batchDelete = async () => {
  ...
  try {
    ...
    const response = await useFetchApi<BaseResponse<any>>('/taxs-delete', 'batch-delete-tax', 'post', ids);
    if(response.status.value == 'success'){
      ElMessage.success('Pajak berhasil dihapus')
      refreshNuxtData('tax')
    }
...
const fetchData = async () => {
  loading.value = true;
  try {
    const response = await useFetchApi<ResponsePagination<Tax[]>>('/search', 'tax', 'post', request_search.value);
    if(response.status.value == 'success'){
...
const onFilter = () => {
  ...
  try {
    ...
    if (ruleFormFilter.tax_value_min !== null || ruleFormFilter.tax_value_max !== null) {
    ...
    if (ruleFormFilter.created_date && ruleFormFilter.created_date.length === 2) {
      request_search.value.filter = {
        ...request_search.value.filter,
        created_at: {
          min: Math.floor(new Date(ruleFormFilter.created_date[0]).getTime() / 1000),
          max: Math.floor(new Date(ruleFormFilter.created_date[1]).getTime() / 1000),
        }
      }
    }
