  ...
  <TrumsWrapper>
    ...
    <el-row :gutter="20" class="mb-3">
      ...
      <el-button
        ...
        :loading-icon="Eleme"
        :loading="loading"
        @click="() => refreshNuxtData('get-addresses')"
      >
        Muat Ulang
      </el-button>
      <el-button 
...
// Data state
const { data } = await useFetchApi<ResponsePagination<AddressType[]>>('/search', 'get-addresses', 'post', request_search.value);
...
const totalProvinces = computed(() => {
  const provinces = new Set((data.value?.data ?? []).map(address => address.province));
