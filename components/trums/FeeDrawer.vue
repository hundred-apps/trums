...
import { type AdjustmentTransaction, FeeType, type ReferenceTransactionAdjustment, ReferenceAdjustment, PartyType } from '~/types/attribute_adjustment'
...
const emit = defineEmits<{
  ...
  (e: 'save', payload: { item: any; contacts: ReferenceTransactionAdjustment[] }): void
}>()
...
<template>
  <el-drawer v-model="visible" title="Pembagian Fee" size="60%">
    <div v-if="item">
      ...
      <div class="mb-3">
        <el-button type="primary" @click="addContact()">+ Tambah Kontak</el-button>
      </div>
      ...
      <el-table :data="contactsFee" border style="width: 100%">
        ...
        <el-table-column label="Fee %">
          <template #default="{ row }">
            <el-input-number :disabled="readonly == true" v-model="row.amount" :min="0" :max="100" />
          </template>
        </el-table-column>

        <el-table-column label="Aksi" width="100">
          <template #default="{ $index }">
            <el-button type="danger" link :disabled="readonly == true" @click="removeContact($index)">
              Hapus
            </el-button>
          </template>
        </el-table-column>
